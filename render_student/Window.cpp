	...
	mDrawSpringforce(false), mDrawEndEffectors(false), mDrawEndEffectorTargets(false), mDrawShadow(true),mMuscleNNLoaded(false),mDrawFigure(false),mDrawCompositionforces(false)
{
	...
	p::exec("from StudentPolicy import *",mns);
...
{
	mNNLoaded = true;
	boost::python::str str = ("num_state_history = "+std::to_string(mEnv->GetNumStateHistory()+mEnv->GetNumRootInfo())).c_str();
	p::exec(str,mns);
	...
	p::exec(str,mns);
	nn_module = p::eval("StudentNN(num_state_history, num_state,num_action)",mns);
...
void
Window::
draw() // updating the view matrix, retrieving information about the environment, and rendering various elements based on the current state and settings.
{	
	GLfloat matrix[16];
...
{	
	...
	{ 
		...
		/////////////////////////////////////////////////////////////////////////////////////////////
			...
			std::map<std::string, std::string> a2i = {{"color","red"}, {"label","imitation_reward"}};
			...
			{ 
				...
				plt::plot(time_vector,foot_clearance_reward_vector,a_p);
				...
				plt::plot(time_vector, action_hip_vector, a19);
				...
				std::vector<double> sum_reward_vector;
				for (size_t i = 0; i < time_vector.size(); ++i) {
					sum_reward_vector.push_back(pos_reward_vector[i] + ee_reward_vector[i]);
				}
				plt::figure(2);
				plt::clf();
				// plt::title("Imitation Reward: model VS reference");
				plt::xlabel("Time/s");
				plt::ylabel("Imitation Reward");
				...
				plt::plot(time_vector,sum_reward_vector,a2i);
				plt::legend(loc);
...
GetActionFromNN()
{
    ...
    // std::cout << "numcurstate   " << numcurstate << std::endl;
	// std::cout << "numcurstate   " << numcurstate << std::endl;
	// std::cout << "numstatehistory   " << numstatehistory << std::endl;
	// Eigen::VectorXd _state = mEnv->GetFullObservation().head(numstates-numtargets);
	Eigen::VectorXd _state_history = mEnv->GetFullObservation().head(numstatehistory);
    
	Eigen::VectorXd state1 = mEnv->GetFullObservation().head(numstates-numtargets);
	Eigen::VectorXd state2 = state1.tail(state1.rows()-numstatehistory);

	p::tuple shape1 = p::make_tuple(_state_history.rows());
	np::dtype dtype1 = np::dtype::get_builtin<float>();
	np::ndarray _state_history_np = np::empty(shape1,dtype1);
	float* dest1 = reinterpret_cast<float*>(_state_history_np.get_data());
	for(int i =0;i<_state_history.rows();i++)
		dest1[i] = _state_history[i];

	p::tuple shape2 = p::make_tuple(state2.rows());
	np::dtype dtype2 = np::dtype::get_builtin<float>();
	np::ndarray state2_np = np::empty(shape2,dtype2);
	float* dest2 = reinterpret_cast<float*>(state2_np.get_data());
	for(int i =0;i<state2.rows();i++)
		dest2[i] = state2[i];

	p::object temp = get_action(_state_history_np,state2_np);
...
//////////////////////////////////////////////// calculate the COP(center of pressure)
	...
	/////////////////////////////////////// save data to .txt 2/22///////////////////////////////////////////////////////
    		...
    		std::ofstream contact_force_vector_l_height_txt("contact_force_vector_l_height_student.txt");
